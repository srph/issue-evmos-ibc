...
// import { EthAccount } from '@tharsis/proto/ethermint/types/v1/account.js'
// import { EthAccount } from './evmos-parser'
...
export function strideAccountParser(input: Any): Account {
  ...
  switch (typeUrl) {
    ...
    case "/ethermint.types.v1.EthAccount": {
      const baseAccount =
        ethermint.types.v1.EthAccount.deserialize(value)?.base_account;
      assert(baseAccount);
      const decoded: BaseAccount = {
        address: baseAccount.address,
        accountNumber: Long.fromValue(
          baseAccount.account_number ? baseAccount.account_number : 0
        ),
        sequence: Long.fromValue(
          baseAccount.sequence ? baseAccount.sequence : 0
        ),
      };
